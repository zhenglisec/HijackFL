...
class Task:
    ...
    def make_optimizer(self, model=None, hijack=False) -> Optimizer:
        if model is None:
            model = self.model
        
        if hijack:
            lr = self.params.hijack_lr
            # decay = self.params.hijack_decay
            # momentum = self.params.hijack_momentum
        else:
            lr = self.params.lr
    ...
    def set_input_shape(self):
        ...
        self.params.input_shape = (3, 32, 32) #inp.shape
    ...
    def report_metrics(self, step, prefix='', logger=None):
        metric_text = []
        for metric in self.metrics:
