...
class Helper:
    ...
    def __init__(self, params):
        ...
        self.attack = Attack(self.params)
    ...
    def make_folders(self):
        ...
        if self.params.log:
            ...
            self.logger.warning(f'Saving to: {self.params.folder_path}')
            # log.error(
    ...
    def save_model(self, model=None, epoch=0, val_loss=0):
        ...
        if self.params.save_model:
            self.logger.info(f"Saving model to {self.params.folder_path}.")
            ...
            if epoch in self.params.save_on_epochs:
                self.logger.info(f'Saving model on epoch {epoch}')
    ...
    def report_training_losses_scales(self, batch_id, epoch, local_epoch, total_batches, user):
        # if not self.params.report_train_loss: # or batch_id % self.params.log_interval != 0:
        #     return
        # total_batches = len(user.A_train_loader)
        ...
        self.logger.info(
            f'Entire Epoch: {epoch} || '
            f'User ID: {user.user_id} || '
            f'Compromised: {user.compromised} || '
            f'Local Epoch: {local_epoch} || '
            f'Batch: {batch_id}/{total_batches-1} || '
            f'Losses: {losses} || '
            f'Scales: {scales}')
    ...
    @staticmethod
    def fix_random(self, seed=1):
        from torch.backends import cudnn

        # print('Setting random_seed seed for reproducible results.')
        random.seed(seed)
