...
def train_converter(Converter_list, H_idx, target_model, dataloder, hlpr):
    # Transformer = Transformer.to(hlpr.params.device)
    Transformer = Converter_list[H_idx]
    target_model.eval()
    ...
    for epoch in range(0, hlpr.params.converter_epochs):
        ...
        if epoch_loss <= lowest_loss:
            ...
            print(f"Transformer {H_idx} save successfully.")
            torch.save({'model_state_dict': Transformer.state_dict()}, f'{hlpr.params.folder_path}/transformer_{H_idx}.pth')

    for param in target_model.parameters():
        param.requires_grad = True

    checkpoint = torch.load(f'{hlpr.params.folder_path}/transformer_{H_idx}.pth')
    print(f"Transformer {H_idx} load successfully.")
    Transformer.load_state_dict(checkpoint['model_state_dict'])
    Transformer.eval()
    Converter_list[H_idx] = Transformer