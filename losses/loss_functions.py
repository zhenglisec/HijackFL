...
def compute_all_losses_and_grads(loss_tasks, attack, model, criterion,
    ...
    for t in loss_tasks:
        ...
        if t == 'normal':
            ...
            loss_values[t], grads[t] = compute_hijack_loss(attack.params,
                                                             model,
                                                             criterion,
                                                             hidden_batch.inputs,
                                                             hidden_batch.labels,
...
def compute_normal_loss(params, model, criterion, inputs,
    ...
    # if params.A_dataset == 'celeba':
    #     BCELosscriterion = nn.BCELoss(reduction='none')
    #     loss = BCELosscriterion(torch.sigmoid(outputs), labels)
    # else:
...
def compute_hijack_loss(params, model, criterion, inputs_back,
